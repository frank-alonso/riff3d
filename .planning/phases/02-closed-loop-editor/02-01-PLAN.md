...
<task type="auto">
  ...
  <action>
    ...
    **Supabase client setup** (follow @supabase/ssr patterns for Next.js 16 App Router):
    ...
    - `lib/supabase/middleware.ts`: `updateSession()` function that creates a server client in middleware context and calls `supabase.auth.getUser()` (NOT `getSession()`) to validate JWT. Redirect unauthenticated users to `/login` for protected routes (`/editor/*`, `/dashboard`). Allow `/login` and `/auth/callback` through.
    ...
    **Shareable project links (PROJ-03):** The URL pattern `/editor/[projectId]` IS the shareable link. For public projects (`is_public = true`), the RLS policy allows read access. No additional linking infrastructure needed for Phase 2.
  </action>
  <verify>
  ...
  <done>
    Auth flow compiles (social login -> callback -> session). Dashboard renders project cards from Supabase data. New project modal creates a project with empty ECSON. Editor route exists at /editor/[projectId]. Dark theme applied globally. CF-05 resolved.
  </done>
</task>
...
- PROJ-01, PROJ-02, PROJ-03 requirements addressed
</success_criteria>
