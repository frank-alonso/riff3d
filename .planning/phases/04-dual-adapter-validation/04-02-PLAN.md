...
<task type="auto">
  ...
  <action>
    ...
    **2. Update viewport-canvas.tsx subscriber (CRITICAL WIRING):**

    The current subscriber in `viewport-canvas.tsx` watches `canonicalScene` and calls `adapter.rebuildScene()`. This needs to be delta-aware:
    ...
    docUnsub = editorStore.subscribe(
      ...
      (canonicalScene) => {
        ...
        if (lastDelta && lastDelta.type !== "full-rebuild") {
          ...
          // Update entity map references in case delta changed structure
    ...
    NOTE: Entity map update after delta is defensive -- for non-structural deltas the map doesn't change, but it's cheap and safe.

    Wait -- `viewport-canvas.tsx` is NOT in this plan's files_modified. It will be modified in 04-03 (engine switching). For now, the delta wiring in scene-slice is sufficient. The viewport subscriber will read `lastDelta` when it reacts to `canonicalScene` changes.

    Actually, the viewport-canvas.tsx subscriber MUST be updated for delta to work. Add it to this task's scope:

    Modify the `editorStore.subscribe` call in viewport-canvas.tsx that watches `canonicalScene`. Change from unconditional `rebuildScene` to checking `lastDelta`:
    - If `lastDelta` exists and is not `full-rebuild`, call `adapter.applyDelta(lastDelta)`
    - Otherwise, call `adapter.rebuildScene(canonicalScene)`

    Import `IRDelta` type (type-only) and `computeDelta` is already used in scene-slice, so no new runtime dep needed in the editor app.
  ...
  <done>
    Scene-slice computes deltas from PatchOps. Viewport subscriber routes deltas to adapter.applyDelta() instead of full rebuild for property changes. Both adapters have tested delta application logic. Structural changes still trigger full rebuild.
  </done>
</task>
