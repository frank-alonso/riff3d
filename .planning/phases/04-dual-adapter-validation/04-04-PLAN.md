...
<task type="auto">
  ...
  <action>
    ...
    **5. Multi-seed property tests (CF-P3-03):**
    ...
    **6. Create `scripts/check-adapter-loc.sh`:**
    ...
    **7. Update conformance package.json:**
...
<task type="auto">
  ...
  <action>
    ...
    **2. Create `apps/editor/e2e/dual-adapter.visual.ts`:**
    ...
    for (const fixture of FIXTURES) {
      for (const engine of ENGINES) {
        test(`${fixture} renders correctly on ${engine}`, async ({ page }) => {
          // Navigate to editor with fixture loaded and engine set
          await page.goto(`/editor/test-project?fixture=${fixture}&engine=${engine}`);
    ...
    For the test project route: the existing E2E test infrastructure should have a way to load fixtures. Check the existing `fixture-render.visual.ts` for the pattern and follow it.
