...
must_haves:
  truths:
    ...
    - "Old-format PatchOps are auto-migrated on load via migrateOp() with a logged warning (LOCKED DECISION)"
  artifacts:
    ...
    - path: "packages/patchops/src/validation.ts"
      ...
      exports: ["validateOp"]
    - path: "packages/patchops/src/migrations/migrate-op.ts"
      provides: "PatchOp format migration runner -- auto-migrates old-format ops on load with logged warning"
      exports: ["migrateOp", "MIGRATION_REGISTRY"]
  key_links:
...
<task type="auto">
  <name>Task 2: Implement PatchOps engine (applyOp, inverse generation, validation, migration runner) with TDD</name>
  <files>
    packages/patchops/src/engine.ts
    packages/patchops/src/validation.ts
    packages/patchops/src/migrations/migrate-op.ts
  ...
  <action>
    ...
    **RED phase -- write failing tests:**
    ...
    `migrate-op.test.ts` (migrateOp function):
    - current-version op passes through unchanged (returns identical PatchOp)
    - console.warn is emitted when a migration applies (spy on console.warn)
    - unparseable input throws ZodError (invalid raw object fails schema validation)
    ...
    **GREEN phase -- implement:**
    ...
    3. **migrations/migrate-op.ts** (per LOCKED DECISION: "Old-format PatchOps auto-migrated on load with a logged warning"):
       ```typescript
       // Migration registry: maps fromVersion -> migration function
       export const MIGRATION_REGISTRY: Map<number, (raw: unknown) => unknown> = new Map();
        ...
        * Logs a warning when migration is applied. ECSON document is the safety net.
        * Returns the migrated op, or the original if already current version.
        */
       export function migrateOp(rawOp: unknown): PatchOp {
         // 1. Extract version from raw op (default to CURRENT if missing)
         // 2. If version < CURRENT_PATCHOP_VERSION, walk migration chain
         // 3. Log warning: `console.warn(\`PatchOp migrated from v\${from} to v\${CURRENT_PATCHOP_VERSION}\`)`
         // 4. Parse with PatchOpSchema and return
       }
       ```
       This establishes the migration infrastructure so that future format changes have a clear upgrade path. The v1 registry is empty but the runner is in place.

    4. **Update index.ts**: Export `applyOp`, `applyOps`, `validateOp`, `migrateOp`.
  ...
  <done>
    The PatchOps engine is fully implemented: `applyOp` handles all 16 op types, returns inverses, validates before applying (including circular reparent detection). Apply-inverse identity is proven for every op type. BatchOp works recursively. The engine mutates ECSON documents deterministically. The `migrateOp()` runner is in place with an empty v1 migration registry, ready for future format version upgrades per the locked decision on auto-migration.
  </done>
</task>
...
<success_criteria>
The @riff3d/patchops package fully implements the PatchOps contract: 16 deterministic, serializable, invertible operation types. The apply engine mutates ECSON documents safely with validation. Every op's inverse is proven correct by tests. Origin categories, format versioning, and PatchOp migration infrastructure are in place.
</success_criteria>
