...
must_haves:
  truths:
    - "6 golden fixtures exist (5 clean + 1 adversarial), all produced by builder API, all valid per Zod"
...
<objective>
Create the golden fixture suite (5 clean + 1 adversarial) using a fluent builder API, implement the conformance harness with round-trip tests, PatchOps replay determinism tests, fast-check property-based tests, and performance benchmark infrastructure. Reference the completed Rapier physics evaluation spike.
...
Output: The `@riff3d/fixtures` package exports builder API and 6 fixture generators. The `@riff3d/conformance` package exports harness, round-trip, replay, benchmark utilities. All conformance tests pass. Rapier spike findings are documented and referenced.
</objective>
...
<task type="auto">
  <name>Task 1: Implement builder API and create 6 golden fixtures with hand-authored reference</name>
  <files>
    packages/fixtures/src/builders/builder.ts
    packages/fixtures/src/builders/transforms-parenting.ts
    packages/fixtures/src/builders/materials-lights.ts
  ...
  <action>
    ...
    2. **5 Clean Fixtures** (one builder file each):
    ...
    6. **Builder tests** (`packages/fixtures/__tests__/builders.test.ts`):
       - Each of the 6 builders produces a valid SceneDocument (Zod parse succeeds)
    ...
    7. **Barrel exports**: `builders/index.ts` and `fixtures/src/index.ts` export all builders and the builder API.
  </action>
  <verify>
    ...
    All 6 fixtures produce valid SceneDocuments per Zod.
    Reference equivalence test confirms builder matches hand-authored JSON.
  </verify>
  <done>
    6 golden fixtures exist (5 clean + 1 adversarial), all produced by the fluent builder API, all Zod-validated. A hand-authored reference fixture documents the format and a test proves builder equivalence. The builder API supports nested fluent construction that flattens to ECSON's entity record format.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    4. **Round-trip tests** (`packages/conformance/__tests__/round-trip.test.ts`):
       - Test each of the 6 golden fixtures through ECSON -> IR -> ECSON round-trip
  ...
  <verify>
    ...
    Round-trip tests pass for all 6 golden fixtures.
    Property tests run at least 100 iterations per property.
    All tests pass deterministically in CI (fixed seed).
  </verify>
  <done>
    Conformance harness is operational with round-trip tests (all 6 fixtures pass), replay determinism tests, and fast-check property tests proving apply-inverse identity, replay determinism, batch equivalence, and structural integrity for random op sequences. The testing spine is proven.
  </done>
</task>
...
- All 6 golden fixtures round-trip through ECSON -> IR -> ECSON
